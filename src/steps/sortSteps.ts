...
When("I sort products by {string}", async function (sortType: string) {
    await page.waitForSelector(".product_sort_container");

    // Retrieve options
    const options = await page.$$eval(".product_sort_container option", (els) =>
        els.map((el) => ({
    ...
    if (!targetValue) {
        throw new Error(
            `❌ Unknown sort type "${sortType}". Available: ${Object.keys(map).join(", ")}`
        );
    }
...
-
Then(
    "products should be sorted in {string} order by {string}",
    async function (direction: string, sortType: string) {
        const isAscending = direction.toLowerCase().includes("asc");

        if (sortType.toLowerCase().includes("price")) {
            const prices: number[] = await page.$$eval(".inventory_item_price", (els) =>
            ...
            const names: string[] = await page.$$eval(".inventory_item_name", (els) =>
